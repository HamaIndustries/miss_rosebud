...
async def wishify(client, message):
    ...
    if len(message.attachments) > 0:
        ...
        await client.send_message(
            message.channel, "Please specify an image to wishify."
        )
        return
    ...
    try:
        ...
        await client.send_message(
            message.channel,
            "It's too big for me! \_(´ཀ`」 ∠) ...If you pay the escort fee, we can try again in the back rooms.",
        )
...
async def dlemoji(client, message):
    response = requests.get(
        discord.utils.get(
            client.get_all_emojis(), name=message.content.split(":")[1]
        ).url
    )
...
async def dlprofile(client, message):
    ...
    response = requests.get(
        ...
        else target.default_avatar_url
    )
...
async def nikki(client, message):
    ...
    if profile.size[1] > 200:
        ...
        profile = profile.resize(
            tuple([x * aspect for x in profile.size]), Image.BICUBIC
        )
...
async def created_by_hideo_kojima(client, message):
    ...
    overlay = overlay.resize(
        (int(overlay.size[0] * ratio), int(overlay.size[1] * ratio)), Image.ANTIALIAS
    )
    profile.paste(
        overlay,
        (
            profile.size[0] - overlay.size[0] - int(100 * ratio),
            profile.size[1] - overlay.size[1] - int(50 * ratio),
        ),
        overlay,
    )
        ...
        if len(message.attachments) > 0:
            for i in message.attachments:
                async with await aiohttp.get(i["url"]) as response:
                    Image.open(BytesIO(await response.content.read())).save(
                        "{}.png".format(a)
                    )
                    a += 1
