...
async def kick(client, message):
    try:
        ...
        await client.send_message(
            message.channel,
            "The bouncers have been notified. {}{} has been escorted accordingly.".format(
                message.mentions[0].name,
                " and company" if len(message.mentions) > 1 else "",
            ),
        )
    except AssertionError:
        await client.send_message(
            message.channel,
            "This command is only authorized for admins against people with roles lower than mine.",
        )
    except discord.Forbidden:
        await client.send_message(
            message.channel,
            "You haven't given me that power. Command only authorized for admins against people with roles lower than mine.",
        )


@roseworks.admincommand(
    "getroleusers",
    "getroleusers [role] {roles...} (Use quotes for multi-word arguments)",
    roseworks.ADMIN,
)
