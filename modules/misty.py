...
async def vore(client, message):
    if len(message.mentions) < 1:
        await client.send_message(message.channel, 'Please specify a user to satisfy your fetishes with.')
        return
    ...
    if target.id == client.user.id:
        if message.author.id == '352229851576401930': #seth
            ...
            await client.send_message(message.channel, '**how\'s my esophagus feel?** (Check into the back rooms if you\'d like to see more ;) )')
            ...
            await client.send_message(message.channel, 'Queen Wishi\'s dick is too big to fit in your mouth!')
    else:
        await client.send_message(message.channel, '{} vored {}. I hope they remembered to buy lube from the lobby desk. ❁_❁'.format(message.author.mention, target.mention))
...
async def weewoo(client, message):
    ...
    wee = await client.send_message(message.channel, str(owo) + ' You\'we undew awwest!!')
    for i in range(3):
