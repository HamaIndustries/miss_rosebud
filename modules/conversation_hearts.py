...
async def c_communicate(client, message):
    async def display_message(channelid, message_type="Message"):
        ...
        e.set_author(
            ...
            else message.author.default_avatar_url,
        )
        e.add_field(
            name=message_type,
            value="[image]" if message.content == "" else message.content,
        )

        if len(message.attachments) > 0:
            e.set_image(url=message.attachments[0]["url"])
        await client.send_message(
            client.get_channel(channelid), content=message.author.id, embed=e
        )
    ...
    async def reply_message(inp):
        ...
        print(
            "==="
            + client.user.name
            + " to {} ( {} ): ".format(target.name.translate(trans), reply[0])
            + reply[1]
        )
        return target.name
    ...
    if message.channel.is_private and not message.author.bot:
        ...
        message.channel.id == general_convo or message.channel.id == dmchannel
        ...
        message.channel.id == Channellistener.listen_channel and not message.author.bot
    ):
