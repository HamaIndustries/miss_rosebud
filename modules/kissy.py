    ...
    try:
        if message.author.id == wishid:
            await client.send_message(
                message.channel,
                "I've added {} {} to your account, Queen Wishi. Please don't hesitate to ask for more, if it pleases you.".format(
                    await target.daily(), Profile.currency_name
                ),
            )
            return
        await client.send_message(
            message.channel,
            "Added {} {} to your account for today. Ask Miss Celosia for chips!".format(
                await target.daily(), Profile.currency_name
            ),
        )
        ...
        await client.send_message(
            message.channel,
            "Wait {:.2f} hours before begging for more {}.".format(
                e.waittime.seconds / 3600.0, Profile.currency_name
            ),
        )
    ...
    if payer.id == wishid:
        ...
        await client.send_message(
            ...
            "Queen Wishi has amended {}'s balance by {}{}.".format(
                message.mentions[0].mention, amount, Profile.currency_symbol
            ),
        )
    elif amount <= 0:
        await client.send_message(
            message.channel, "You can't pay {} fool.".format(amount)
        )
        return
    elif payer.id == recipient.id:
        await client.send_message(
            message.channel,
            "Trying to defraud the casino? You should know better than to risk what's most important to you.",
        )
        return
    elif message.mentions[0].bot:
        await client.send_message(
            message.channel,
            "The only 'bot who can be paid off is me. Meet me in room 7A in the next five minutes and I might charge you less.",
        )
        return
        ...
        await client.send_message(
            message.channel,
            "You can't afford to pay that. A round on the tables, perhaps?",
        )
...
async def bal(client, message):
    e = discord.Embed(
        title="{}'s balance".format(message.author.name.translate(trans)),
        description="{}{}".format(
            Profile.currency_symbol, Profile(message.author).get_balance()
        ),
    )
...
async def rank(client, message):
    ...
    async for profile in profiles.get_all_profiles(memberlist):
        if profile.user.bot or profile.id in dup_detector or profile.id == wishid:
            continue
        ...
        ranking.append(
            [
                ...
                profile.get_balance(),
                0
                if isinstance(profile.info["wishimarriages"], int)
                else profile.info["wishimarriages"]["marriages"],
            ]
        )
    ...
    ranking.insert(
        ...
        [
            wprofile.name,
            wprofile.get_balance(),
            0
            if isinstance(profile.info["wishimarriages"], int)
            else wprofile.info["wishimarriages"]["marriages"],
        ],
    )
    e = discord.Embed(
        title="Top {} users for {} by {}:".format(
            length,
            "all servers" if all else "this server",
            "wishimarriages" if wishi else "wealth",
        ),
        color=utils.p_pink,
    )
    for i in ranking[:length]:
        e.add_field(
            name=i[0],
            value="{}: {}, Wishi Marriages: {}".format(
                Profile.currency_name.capitalize(), i[1], i[2]
            ),
            inline=False,
        )
    await client.send_message(message.channel, embed=e)
